...
| `validate_bus_aliases` | Check every `PREFIX{ALIAS}` label and sheet pin in the hierarchy against the aliases actually defined in it. Names the alias and the file for a reference nothing defines — ERC catches that case too, but as a scatter of `net_not_bus_member` and `isolated_pin_label` violations naming neither. Also reports one alias defined twice with different members, which ERC has no opinion about, and unused definitions. Read-only. |
